...
const menu = [
  { href: "/landfill", label: "არსებული ნაგავსაყრელები" },
  { href: "/landfill/new", label: "ახალი ობიექტები" },
  { href: "/landfill/stations", label: "გადამტვირთავი სადგურები" },
];

export default function LandfillLayout({ children }) {
  ...
  const isActive = (href) => {
    if (href === "/landfill") {
      return (
        pathname === "/landfill" ||
        (pathname.startsWith("/landfill/") &&
          !pathname.startsWith("/landfill/new") &&
          !pathname.startsWith("/landfill/stations"))
      );
    }
  ...
  const currentTitle =
    menu.find((item) => isActive(item.href))?.label || "ობიექტები";
