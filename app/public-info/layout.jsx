...
const menu = [
  { href: "/public-info", label: "მოითხოვე საჯარო ინფორმაცია" },
  { href: "/public-info/tenders", label: "ტენდერები" },
  { href: "/public-info/jobs", label: "ვაკანსიები" },
];

export default function PublicInfoLayout({ children }) {
  ...
  const isActive = (href) => {
    if (href === "/public-info") {
      return (
        ...
        (pathname.startsWith("/public-info/") &&
          !pathname.startsWith("/public-info/tenders") &&
          !pathname.startsWith("/public-info/jobs"))
      );
    }
  ...
  const currentTitle =
    menu.find((item) => isActive(item.href))?.label || "საჯარო ინფორმაცია";
    ...
    <section className="py-10 bg-white space-y-10">
      <div className="max-w-[1640px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[2fr_1fr] gap-10">
          ...
          {/* CONTENT */}
          <div className="order-2 lg:order-1 w-full max-w-[800px] mx-auto lg:max-w-none">
            {children}
          </div>
